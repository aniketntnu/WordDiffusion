...
def train(diffusion, model, ema, ema_model, vae, optimizer, mse_loss, loader, num_classes, vocab_size, transforms, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (images, word, s_id,label) in enumerate(pbar):
...
def main():
    ...
    parser.add_argument('--batch_size', type=int, default=432)
    ...
    parser.add_argument('--gt_train', type=str, default=['./gt/results_IAM_train.filter27',"./gt/gan.iam.tr_va.gt.filter27"][1]) #  
    #UNET parameters
    ...
    parser.add_argument('--csvRead', type=str, default="/cluster/datastore/aniketag/newHTR/HTR-best-practices/allResults/IAM/resultsTrainForDiffusion.csv", help='training info from .csv instead of authors file')
    # "/cluster/datastore/aniketag/newHTR/HTR-best-practices/allResults/IAM/resultsTrainForDiffusion.csv"
    
    parser.add_argument('--trascriptionPlusOCR', type=int, default=1,help = "it joins transcription and OCR prediction as a conditional input")
    
    parser.add_argument('--save_path', type=str, default="./models/Mse_Transcription_OcrPred_FromScratch/",help = "this is location where it savesthe new model" ) 
    parser.add_argument('--saveModelName', type=str, default="Transcription_OcrPred_FromScratch.pt",help = "by this name save model at save_path" ) 
