...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.FileHandler('//cluster/datastore/aniketag/newWordStylist/WordStylist/logs/icdar2025/PhoscLogs.log'),  # Add a FileHandler
        logging.StreamHandler()  # Add a StreamHandler for console output
    ]
)
#logger = logging.getLogger('cvlGenerationUnetPhosc')
#logger.info('--- cvlGenerationUnetPhosc ---')
...
class UNetModelPhosc(nn.Module):
    """
    ...
    def forward(self, x,phoscLabels, timesteps=None, context=None, y=None):
        ...
        if self.interpolation:
            ...
            try:
                ...
                print("\n\t emb.shape:",emb.shape," y.shape:",y.shape)
                print("\n\t y:",y)
                print("\n\t exception is ",e)
        ...
        #print("%%%%%:",currTime)
        if (context is not None and self.args.phosc==1) or (context is not None and self.args.phos==1):
            
            #print("\n\t phoscLabels.shape:",phoscLabels.shape)
            
            phoscLabels = phoscLabels.int().to(context.device)#.to(context.dtype)
            contextPhosc = self.word_emb(phoscLabels)

            #print("\n\t 1.context.shape:",context.shape,"\t phosc.shape:",phoscLabels.shape," \t contextPhosc.shape:",contextPhosc.shape)

            if self.args.phosc==1 or self.args.phos==1:
                context = torch.concat([context,contextPhosc],axis = 1)
            else:
                pass
                #context = contextPhosc
            #print("\n\t 2.context.shape:",context.shape,"\t phosc.shape:",phoscLabels.shape)


        
        h = x.type(self.dtype)
        # h, emb, context
        log_with_line_number(f"\t 6.3 input_blocks h.shape: {h.shape} emb.shape: {emb.shape} context.shape: {context.shape}")
        #INPUT BLOCKS
        for module in self.input_blocks:
            h = module(h, emb, context)
            hs.append(h)

        log_with_line_number(f"\t 6.4 MIDDLE BLOCK")

        
        #MIDDLE BLOCK
        h = self.middle_block(h, emb, context)

        log_with_line_number(f"\t 6.5 OUTPUT BLOCKS")
        ...
        log_with_line_number(f"\t 6.6 OUTPUT BLOCKS end")
