...
import pandas as pd
import shutil
from utils.dataGenerationConfigICPR import *
...
logging.basicConfig(
    ...
    level=logging.INFO,
    handlers=[
        logging.FileHandler('//cluster/datastore/aniketag/newWordStylist/WordStylist/logs/delMe2.log'),  # Add a FileHandler
        logging.StreamHandler()  # Add a StreamHandler for console output
    ]
)
logger = logging.getLogger('wordStylistGenerationLogs')
logger.info('--- wordStylistGenerationLogs ---')
...
if lang == "ENG":
    ...
    #c_classes = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_'
    c_classes = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz' # icpr
...
def createDataLoader(args,transforms):
    ...
    if 1:#os.path.isdir(dumpBasePath+split+batchFolder):
        ...
        #genAcceptImageList = os.listdir("///cluster/datastore/aniketag/allData/syntheticData/train/icpr//icprRebuitalWordStylist1//") #os.listdir(dumpBasePath+split+batchFolder)

        genAcceptImageList = os.listdir("///cluster/datastore/aniketag/allData/syntheticData/train///delMe//") #os.listdir(dumpBasePath+split+batchFolder)
    ...
    with open(args.gtPath, 'r') as f:
        ...
        if args.lang == "ENG":
            #style_classes= 330 #339#len(wr_dict)
            style_classes= 339 #339#len(wr_dict)
...
class IAMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args.imgConditioned == 1:
            img_path = os.path.join(self.image_path, image_name+".png")
    ...
    def sampling3(self,epoch,x_t,words,phoscLabels, model,model1, vae,emaOld,noiseInput, n, x_text, labels, args, mix_rate=None, cfg_scale=3):
        ...
        with torch.no_grad():
            ...
            for i in reversed(range(1, self.noise_steps)):
                ...
                # /home/aniketag/Documents/phd/TensorFlow-2.x-YOLOv3_simula/Handwriting-1-master/PapersReimplementations/newWordStylist/WordStylist/dump
                else:
                    pass
...
def train(diffusion, model,net, ema, ema_model,ema_model1, vae, optimizer, mse_loss, loader, num_classes, vocab_size, transforms, args):
    ...
    #dumpPath = "/cluster/datastore/aniketag/allData/syntheticData/train/icpr//icprRebuitalWordStylist1/"

    dumpPath = "/cluster/datastore/aniketag/allData/syntheticData/train//delMe3/"
    ...
    for epoch in range(args.epochs):
        ...
        for i, (image,image_name,word, s_id, wordLabel,phoscLabels) in enumerate(pbar):
            ...
            if args.ocr ==1:# and random.random()< 0.1 and args.partialCTC: 
                ...
                if args.savedOcrImages:
                    ...
                    logger.info("\n\t writing images at location: %s", writerDumpPath)
                ...
                # scp -r -P 60441 aniket@dnat.simula.no:/global/D1/projects/ZeroShot_Word_Recognition/E2E/allData/WordStylist/authorsModel .
               ...
               # print("\n\t copying and deleting images!!!")
                ...
                logger.info("\n\t correctCount: %d \t totCount: %d \t accuracy: %.2f \t tot accepted: %d \t delCount: %d \t tot images: %d",correctCount, totCount, (correctCount * 1.0 / totCount), len(os.listdir(writerDumpPath)), delCount, len(os.listdir(writerDumpPath)))
            #exit()   
            #input("check!!!")
...
def main():
    ...
    parser.add_argument('--partialLoad',  type=int, default=0.0001)
    ...
    if 1:#os.path.isfile(args.loadPrevPath):
        ...
        print("\n\t Loading OCR model complete from path:",ocrModel)    
    ...
    if args.ddp==1:
        ...
        if args.phosc == 1 or args.phos == 1:
            ...
            print("\n\t Using Unet from wordstylist model!!!")
            unet = UNetModel(image_size = args.img_size, in_channels=args.channels,
                            model_channels=args.emb_dim, out_channels=args.channels,
                            num_res_blocks=args.num_res_blocks, attention_resolutions=(1,1), 
                            channel_mult=(1, 1), num_heads=args.num_heads, num_classes=style_classes,
    ...
    if args.loadPrev == 1 and os.path.isfile(args.authorBasePath+ckptModelName):
        ...
        print("\n\t state_dict keys:",first_five_keys)
    ...
    if args.loadPrev == 1 and os.path.isfile(args.authorBasePath+emaModelName):
        ...
        ema_model.load_state_dict(torch.load(args.authorBasePath+emaModelName,map_location=device),strict= True)
    ...
    if args.loadPrev == 1 and os.path.isfile(args.authorBasePath+emaModelName):
        ema_model1.load_state_dict(torch.load(args.authorBasePath+emaModelName,map_location=device),strict= False)
        print("\n\t ema1 model loaded!!!")
